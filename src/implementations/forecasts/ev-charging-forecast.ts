...
import {
    ...
    EnyoDataBusEvChargingForecastV1,
    EnyoDataBusMessageEnum,
    EnyoEvChargingForecastDataPoint,
...
/**
 * Per-slot EV-charging history payload.
 */
interface EvChargingHistoryValues {
    /** Average charging power in Watts during the slot. */
    powerW: number;
    /** Number of charging-meter-values samples that contributed. */
    samples: number;
}

/**
 * Forecast result returned by {@link EvChargingForecast.getForecast}, shaped
 * to match `EnyoDataBusEvChargingForecastV1.data`.
 */
export type EvChargingForecastResult = BaseForecast<EnyoDataBusEvChargingForecastV1['data']>;

/**
 * Builds a 24-hour EV-charging power forecast for a single charger appliance
 * from historical `ChargingMeterValuesUpdateV1` data-bus messages, with live
 * updates merged in as new sessions report.
 *
 * EV charging follows daily-routine patterns (evening top-up, overnight, etc.),
 * so the algorithm uses a same-weekday recency-weighted average over the
 * configured history window (default 14 d), with an all-weekday fallback for
 * slots that lack same-weekday samples.
 *
 * @example
 * ```ts
 * const forecast = new EvChargingForecast(app, 'charger-1', { source: EnyoSourceEnum.Device });
 * await forecast.initialize();
 * const result = forecast.getForecast();
 * ```
 */
export class EvChargingForecast implements Forecaster {
    /** Charger usage tends to be weekday-cyclic. */
    public static readonly DEFAULT_HISTORY_DAYS = 14;

    private readonly config: ResolvedForecastConfig;
    private readonly history: HistoricalBucket<EvChargingHistoryValues>[] = [];
     ...
     * @param applianceId - Charger appliance ID this forecast covers.
    ...
    constructor(
        ...
        this.config = resolveForecastConfig(
            {historyDays: EvChargingForecast.DEFAULT_HISTORY_DAYS},
            options.config,
        );
    ...
    /**
     * Pulls historical `ChargingMeterValuesUpdateV1` messages from
     * `useTimeseries().queryDataBusMessages()` and aggregates them into
     * 15-minute slots, then starts listening to live charging events.
     * Idempotent.
     */
    public async initialize(): Promise<void> {
        ...
        const response = await this.app.useTimeseries().queryDataBusMessages({
            ...
            messageTypes: [EnyoDataBusMessageEnum.ChargingMeterValuesUpdateV1],
            limit: 100000,
        });

        const slotAggregator = new Map<number, {sum: number; count: number}>();
        for (const raw of response.messages) {
            const message = raw as EnyoDataBusChargingMeterValuesV1;
            const power = message.data.chargingPowerW;
        ...
        this.listenerId = this.app.useDataBus().listenForMessages(
            [EnyoDataBusMessageEnum.ChargingMeterValuesUpdateV1],
            (msg) => this.onChargingMeterValues(msg as EnyoDataBusChargingMeterValuesV1),
        );
    }

    /**
     * Returns the current forecast as an `EnyoDataBusEvChargingForecastV1`-shaped
     * payload over the configured horizon. Slots without any historical samples
     * are predicted as zero power.
     */
    public getForecast(): EvChargingForecastResult {
        const now = Date.now();
        ...
        const entries: EnyoEvChargingForecastDataPoint[] = buckets.map((b) => ({
            timestampIso: b.timestampIso,
            powerW: b.payload.powerW,
            powerWh: b.payload.powerWh,
        }));

        const result: EvChargingForecastResult = {
    ...
    private onChargingMeterValues(message: EnyoDataBusChargingMeterValuesV1): void {
    ...
    private publish(result: EvChargingForecastResult): void {
        const message: EnyoDataBusEvChargingForecastV1 = {
